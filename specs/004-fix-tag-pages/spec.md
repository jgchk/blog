...
- What happens when a tag slug contains special characters (e.g., "C++", "Node.js")?
  - Tag slugs are generated by `normalizeTagSlug()` in @blog/core, which converts special characters to URL-safe equivalents (e.g., "C++" → "c-plus-plus", "Node.js" → "node-js")
  - The rendered page displays the original tag name from TagIndex (e.g., "C++"), not the slug
...
- How does case sensitivity work for tag URLs?
  - Tag URL matching is case-insensitive: the slug from the URL is lowercased before lookup (e.g., `/tags/TypeScript.html` → lookup "typescript")
  - All case variants resolve to identical HTML output
...
- **Tag**: A label applied to articles for categorization. Has a display name (e.g., "Getting Started") and a normalized slug (e.g., "getting-started") generated by `normalizeTagSlug()` in @blog/core

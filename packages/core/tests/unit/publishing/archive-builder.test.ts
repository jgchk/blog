...
import { ArchiveBuilder } from '../../../src/publishing/archive-builder.js';
...
describe('ArchiveBuilder', () => {
  ...
  describe('buildArchive', () => {
    ...
    it('should sort articles within group by date descending', () => {
      ...
      expect(archive[0]?.articles[0]?.slug.toString()).toBe('last');
      expect(archive[0]?.articles[1]?.slug.toString()).toBe('middle');
      expect(archive[0]?.articles[2]?.slug.toString()).toBe('first');
    });
  ...
  describe('getArticlesByMonth', () => {
    it('should return articles for specific year-month', () => {
      ...
      expect(result.map((a) => a.slug.toString())).toContain('a');
      expect(result.map((a) => a.slug.toString())).toContain('b');
    });
